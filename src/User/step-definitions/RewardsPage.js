...
Then(/^Nisa should see Button Name: "([^"]*)" under Confirmed Points in Rewards Page$/, async (text) => {
    await RewardsPage.checkHediyeyiAl(text)
});
Then(/^Nisa should see Button Name: "([^"]*)" above Confirmed Points in Rewards Page$/, async (text) => {
    await RewardsPage.checkPuanKazan(text)
});
