...
Then(/^Nisa should see Membership Date:"([^"]*)" in Tier Status Page$/, async (text) => {
    await TierStatusPage.checkMembershipDate(text)
});
...
Then(/^Nisa should see Privileges Name:"([^"]*)" is checked in Tier Status Page$/, async (text) => {
    await TierStatusPage.checkPrivilegesNameIsChecked(text)
});
